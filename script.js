...
const resetCards = () => {
    deck = Array.from({ length: 52 * NUMBER_OF_DECKS }, (_, i) => {
        let newCard = {
            color: i < (52 * NUMBER_OF_DECKS) / 4 ?
                "hearts" : i < (52 * NUMBER_OF_DECKS) / 2 ?
                "spades" : i < (52 * NUMBER_OF_DECKS) / 2 + (52 * NUMBER_OF_DECKS) / 4 ?
                "diamonds" : "clubs",
        };
        if ((i % 13) + 1 === 1) {
            newCard["symbol"] = "A";
...
const hit = () => {
    ...
    if (Math.min(...userScore) > 21) {
        setTimeout(() => {
            $("body").prepend(createBackdrop("Oh no...", "You bust :( Play again?"));
